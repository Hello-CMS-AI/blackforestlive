...
const EditStockEntry = () => {
  ...
  useEffect(() => {
    ...
    const fetchStockEntry = async () => {
      try {
        const response = await fetch(`http://localhost:5000/api/dealer/stock-entries/${id}`, {
          method: 'GET',
          headers: {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:5000/api/dealer/products?category=${selectedCategory}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
          },
