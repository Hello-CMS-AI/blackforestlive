...
import { Form, Input, Button, message, Row, Col, Select, Upload } from 'antd';
...
const CreateBillEntry = () => {
  ...
  const [errors, setErrors] = useState({ dealer: '', branch: '', billNumber: '', amount: '', billImage: '' });
  ...
  useEffect(() => {
    const fetchBranches = async () => {
      try {
        const response = await fetch('https://api10.theblackforestcakes.com/api/branches/public', { // Updated to public endpoint
          method: 'GET',
          headers: { 'Content-Type': 'application/json' },
        });
        const result = await response.json();
  ...
  const onFinish = async (values) => {
    setLoading(true);
    setErrors({ dealer: '', branch: '', billNumber: '', amount: '', billImage: '' });
  ...
  const onFinishFailed = (errorInfo) => {
    ...
    const newErrors = { dealer: '', branch: '', billNumber: '', amount: '', billImage: '' };
