...
const EditBillEntry = () => {
  ...
  const fetchBillData = async () => {
    setLoading(true);
    try {
      const response = await fetch(`https://api10.theblackforestcakes.com/api/dealers/bills/${id}`, {
        method: 'GET',
      ...
      if (response.ok) {
        ...
        const initialFileList = result.billImage
          ? [{ uid: '-1', name: 'Current Image', status: 'done', url: `https://api10.theblackforestcakes.com/${result.billImage}` }]
          : [];
