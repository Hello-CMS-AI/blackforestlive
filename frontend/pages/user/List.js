...
const UserListPage = () => {
  ...
  const fetchUsers = async (token) => {
    setLoading(true);
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'https://api10.theblackforestcakes.com'}/api/users`, {
  ...
  const fetchBranches = async (token) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'https://api10.theblackforestcakes.com'}/api/branches`, {
        headers: { 'Authorization': `Bearer ${token}` },
      });
      const data = await response.json();
  ...
  const handleToggleStatus = async (userId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'https://api10.theblackforestcakes.com'}/api/users/${userId}/toggle`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${token}` },
      });
      const data = await response.json();
  ...
  const handleDelete = async (userId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'https://api10.theblackforestcakes.com'}/api/users/${userId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` },
      });
      const data = await response.json();
  ...
  const onFinish = async (values) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'https://api10.theblackforestcakes.com'}/api/users/${selectedUser._id}`, {
        method: 'PUT',
        headers: {
